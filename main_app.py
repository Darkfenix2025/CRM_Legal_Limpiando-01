print("=====================================================================")
print("INICIANDO SCRIPT main_app.py - VERSIÓN DE DEBUG FINAL")
print("=====================================================================")
...
class CRMLegalApp:
    def __init__(self, root):
        ...
        filemenu = tk.Menu(menubar, tearoff=0); filemenu.add_command(label="Mostrar Ventana", command=self._mostrar_ventana_callback); filemenu.add_separator(); filemenu.add_command(label="Ocultar a Bandeja", command=self.ocultar_a_bandeja); filemenu.add_separator(); filemenu.add_command(label="Salir", command=self.cerrar_aplicacion_directamente); menubar.add_cascade(label="Archivo", menu=filemenu)
        ia_menu = tk.Menu(menubar, tearoff=0); ia_menu.add_command(label="Reformular Hechos...", command=self.open_reformular_hechos_dialog); menubar.add_cascade(label="Asistente IA", menu=ia_menu)
        adminmenu = tk.Menu(menubar, tearoff=0); adminmenu.add_command(label="Crear Copia de Seguridad...", command=self.crear_copia_de_seguridad); menubar.add_cascade(label="Administración", menu=adminmenu)
    ...
    def open_case_detail_window(self, event=None):
        ...
        if case_id in self.open_case_windows:
            window = self.open_case_windows[case_id]; window.lift(); window.focus_force(); return
    ...
    def on_case_window_close(self, case_id):
        if case_id in self.open_case_windows: del self.open_case_windows[case_id]

    def create_widgets(self):
        print("--- EJECUTANDO create_widgets VERSIÓN FINAL (LOGO EN COL 2, CASOS EN COL 3) ---")
        ...
        # --- Columna 1: Clientes ---
        col1_frame = ttk.Frame(crm_main_frame); col1_frame.grid(row=0, column=0, sticky='nsew', padx=(0, 5)); col1_frame.rowconfigure(0, weight=1); col1_frame.columnconfigure(0, weight=1)
        client_list_frame = ttk.LabelFrame(col1_frame, text="Clientes", padding="5"); client_list_frame.grid(row=0, column=0, sticky='nsew', pady=(0, 5)); client_list_frame.columnconfigure(0, weight=1); client_list_frame.rowconfigure(0, weight=1); self.client_tree = ttk.Treeview(client_list_frame, columns=('ID', 'Nombre'), show='headings', selectmode='browse'); self.client_tree.heading('ID', text='ID'); self.client_tree.heading('Nombre', text='Nombre'); self.client_tree.column('ID', width=40, stretch=tk.NO); self.client_tree.column('Nombre', width=150, stretch=tk.YES); client_scrollbar_y = ttk.Scrollbar(client_list_frame, orient=tk.VERTICAL, command=self.client_tree.yview); self.client_tree.configure(yscrollcommand=client_scrollbar_y.set); client_scrollbar_x = ttk.Scrollbar(client_list_frame, orient=tk.HORIZONTAL, command=self.client_tree.xview); self.client_tree.configure(xscrollcommand=client_scrollbar_x.set); self.client_tree.grid(row=0, column=0, sticky='nsew'); client_scrollbar_y.grid(row=0, column=1, sticky='ns'); client_scrollbar_x.grid(row=1, column=0, sticky='ew'); self.client_tree.bind('<<TreeviewSelect>>', self.on_client_select)
        client_buttons_frame = ttk.Frame(col1_frame); client_buttons_frame.grid(row=1, column=0, sticky='ew', pady=5); self.add_client_btn = ttk.Button(client_buttons_frame, text="ALTA", command=lambda: self.open_client_dialog()); self.add_client_btn.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 5)); self.edit_client_btn = ttk.Button(client_buttons_frame, text="MODIFICAR", command=lambda: self.open_client_dialog(self.selected_client['id'] if self.selected_client else None), state=tk.DISABLED); self.edit_client_btn.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5); self.delete_client_btn = ttk.Button(client_buttons_frame, text="BORRAR", command=self.delete_client, state=tk.DISABLED); self.delete_client_btn.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(5, 0))
        client_details_frame = ttk.LabelFrame(col1_frame, text="Detalles Cliente", padding="10"); client_details_frame.grid(row=2, column=0, sticky='ew', pady=(5, 0)); client_details_frame.columnconfigure(1, weight=1); ttk.Label(client_details_frame, text="Nombre:").grid(row=0, column=0, sticky=tk.W, pady=1, padx=5); self.client_detail_name_lbl = ttk.Label(client_details_frame, text="", wraplength=200); self.client_detail_name_lbl.grid(row=0, column=1, sticky=tk.EW, pady=1, padx=5); ttk.Label(client_details_frame, text="Dirección:").grid(row=1, column=0, sticky=tk.W, pady=1, padx=5); self.client_detail_address_lbl = ttk.Label(client_details_frame, text="", wraplength=200); self.client_detail_address_lbl.grid(row=1, column=1, sticky=tk.EW, pady=1, padx=5); ttk.Label(client_details_frame, text="Email:").grid(row=2, column=0, sticky=tk.W, pady=1, padx=5); self.client_detail_email_lbl = ttk.Label(client_details_frame, text="", wraplength=200); self.client_detail_email_lbl.grid(row=2, column=1, sticky=tk.EW, pady=1, padx=5); ttk.Label(client_details_frame, text="WhatsApp:").grid(row=3, column=0, sticky=tk.W, pady=1, padx=5); self.client_detail_whatsapp_lbl = ttk.Label(client_details_frame, text="", wraplength=200); self.client_detail_whatsapp_lbl.grid(row=3, column=1, sticky=tk.EW, pady=1, padx=5); ttk.Label(client_details_frame, text="Etiquetas:").grid(row=4, column=0, sticky=tk.W, pady=1, padx=5); self.client_detail_tags_lbl = ttk.Label(client_details_frame, text="", wraplength=200); self.client_detail_tags_lbl.grid(row=4, column=1, sticky=tk.EW, pady=1, padx=5)

        # --- Columna 2: Logo y Calendario ---
        col2_frame = ttk.Frame(crm_main_frame); col2_frame.grid(row=0, column=1, sticky='nsew', padx=5); col2_frame.rowconfigure(1, weight=0); col2_frame.columnconfigure(0, weight=1)
        logo_frame = ttk.LabelFrame(col2_frame, text="Powered by"); logo_frame.grid(row=0, column=0, sticky="ew", pady=(0,5), ipady=5); logo_frame.columnconfigure(0, weight=1)
        try:
    ...
    def on_case_select(self, event):
        ...
        if selected_items:
            try:
                ...
                self.selected_case = None; self.disable_case_buttons(); self.add_audiencia_btn.config(state=tk.DISABLED)
        else:
            self.selected_case = None; self.disable_case_buttons(); self.add_audiencia_btn.config(state=tk.DISABLED)
            
    # --- Pega aquí el RESTO de tu código de main_app.py sin cambios ---
    # (open_client_dialog, save_client, delete_client, open_case_dialog, save_case, delete_case,
    # toda la lógica de audiencias, recordatorios, bandeja, etc.)
    # ...
...
# ... (tu `if __name__ == "__main__":` y el `root.mainloop()` al final) ...
    ...
    def _formatear_texto_audiencia_para_compartir(self, audiencia):
        ...
        fecha_dt = datetime.datetime.strptime(audiencia.get('fecha'), "%Y-%m-%d")
