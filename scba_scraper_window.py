    ...
    def open_browser(self):
        try:
            ...
            if "net::ERR_CONNECTION_REFUSED" in str(e) or "This version of ChromeDriver only supports Chrome version" in str(e):
                 print(f"\n[ERROR CRÍTICO] Problema con ChromeDriver o conexión: {e}")
                 print("Por favor, asegúrate de que ChromeDriver esté instalado y sea compatible con tu versión de Chrome.")
                 print("Puedes descargarlo desde: https://chromedriver.chromium.org/downloads")
            else:
            ...
            print(f"\n[ERROR INESPERADO] No se pudo iniciar el navegador: {type(e).__name__}: {e}")
        ...
        try:
            ...
            for i, url in enumerate(urls_to_process):
                if not self.is_driver_active(): # Chequear antes de cada acción
                    print("\n[ERROR] El navegador se cerró inesperadamente.")
                    break
                if url in extracted_urls:
                    print(f" - ({i+1}/{len(urls_to_process)}) Saltando, ya extraído: {url.split('?')[1] if '?' in url else url[-20:]}")
                    continue

                print(f" - ({i+1}/{len(urls_to_process)}) Extrayendo: {url.split('?')[1] if '?' in url else url[-20:]} ...")
            ...
            print(f"\n¡Extracción de lote finalizada! Se extrajeron {new_extractions} nuevos textos.")
             ...
             print(f"\n[ERROR SELENIUM] Ocurrió un error durante la extracción: {type(e).__name__} - {e}")
        except Exception as e:
            print(f"\n[ERROR INESPERADO] Ocurrió un error durante la extracción: {type(e).__name__} - {e}")
            import traceback
